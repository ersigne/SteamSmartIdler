...
class Idler {
     ...
     * @param {Number[]} freePointsShopItems - DefID's of free Steam Points Shop Items
     */
    constructor(account, freeLicenses, freePointsShopItems) {
    ...
    #setStartupEvents() {
        ...
        this.client.on('licenses', (licenses) => {
            ...
            this.ownedLicenses = licenses.map((license) => license.package_id);
    ...
    async #login() {
        ...
        await this.fetchAndSetAccessToken(this.cookies);
    }

    addFreeLicenses(freelicenses) {
        for (const subID of freelicenses) {
            this.store.addFreeLicense(subID, (err) => {
                if (err) console.error(`${this.username} - Add Free License error: ${err}`);
                else console.log(`${this.username} - Added Free License with SubID ${subID}`);
            });
        }
    }

    async addFreeSteamPointsShopItems(freeitems, access_token, cookies) {
        const redeemedItems = await Requester.addFreeSteamPointsShopItems(freeitems, access_token, cookies);
        if (!redeemedItems) return;
        for (const DefID of redeemedItems) {
            console.log(`${this.username} - Added Free Steam Points Shop Item with DefID ${DefID}`);
        }
    }

    async fetchAndSetAccessToken(cookies) {
        const access_token = await Requester.fetchAccessToken(cookies);
    ...
    async start() {
        ...
        await this.addFreeLicenses(this.freeLicenses);
        await this.addFreeSteamPointsShopItems(this.freePointsShopItems, this.access_token, this.cookies);
        await this.#idle();
    }
}
